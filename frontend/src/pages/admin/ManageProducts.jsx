...
import { Trash2, Search, Pencil, Plus, ToggleLeft, ToggleRight } from "lucide-react";
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{pagination.total} total products on the website</p>
        </div>
        <button
                    ...
                    <tr key={p._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <img src={p.image || PLACEHOLDER_IMAGE} alt={p.title} className="w-9 h-9 rounded-lg object-cover bg-gray-100 shrink-0" onError={(e) => { e.target.src = PLACEHOLDER_IMAGE; }} />
                          <span className="font-medium text-gray-900 dark:text-white line-clamp-1 max-w-[160px]">{p.title}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-gray-500 text-xs">{p.farmer?.farmName || p.farmer?.name}</td>
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          className={`badge cursor-pointer transition-colors ${p.isActive ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 hover:bg-red-100 hover:text-red-700" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 hover:bg-green-100 hover:text-green-700"}`}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => navigate(`/dashboard/admin/products/edit/${p._id}`)}
